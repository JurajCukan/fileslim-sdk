...
export async function compressPDF(
  ...
  const maxImageDimension =
    options?.maxImageDimension ?? modeConfig.maxImageDimension;
  ...
  try {
    ...
    if (compressedImages.length > 0) {
      ...
      replaceImagesInPDF(pdfDoc, PDFRawStream, compressedImages);
    }
...
async function decompressFlateDataSDK(
  data: Uint8Array
): Promise<Uint8Array> {
      ...
      try {
        if (obj && typeof obj === 'object' && 'dict' in obj && 'getContents' in obj) {
          ...
          if (
            ...
            const width = dict.has(PDFName.of('Width'))
              ? dict.get(PDFName.of('Width'))?.asNumber()
              : 0;
            const height = dict.has(PDFName.of('Height'))
              ? dict.get(PDFName.of('Height'))?.asNumber()
              : 0;
    ...
    for (let i = 0; i < imageRefs.length; i += BATCH_SIZE) {
      ...
      const batchResults = await Promise.all(
        batch.map(async ({ ref, obj, width, height }) => {
          try {
            const dict = (obj as any).dict;
            const compressedImageBytes = (obj as any).getContents?.();
            if (!compressedImageBytes || compressedImageBytes.length === 0) {
              return null;
            }

            // Determine filter type
            const filter = dict.has(PDFName.of('Filter'))
              ? dict.get(PDFName.of('Filter'))?.toString() ?? ''
              : '';
            ...
            if (isDCT) {
              const imageBlob = new Blob([compressedImageBytes], {
                type: 'image/jpeg'
              });
              ...
              const bpc = dict.has(PDFName.of('BitsPerComponent'))
                ? dict.get(PDFName.of('BitsPerComponent'))?.asNumber() ?? 8
                : 8;
              const cs = dict.has(PDFName.of('ColorSpace'))
                ? dict.get(PDFName.of('ColorSpace'))?.toString() ?? '/DeviceRGB'
                : '/DeviceRGB';
              const components = cs.includes('Gray')
                ? 1
                : cs.includes('CMYK')
                ? 4
                : 3;
              ...
              const expectedSize =
                width * height * components * (bpc / 8);
              if (decompressed.length < expectedSize) return null;

              const rgba = rawPixelsToRGBASDK(
                decompressed,
                width,
                height,
                components
            ...
            try {
              ...
              const encoded = await encode(imageData, {
                quality: Math.round(targetQuality * 100)
              });
...
function replaceImagesInPDF(
  ...
  for (const { ref, compressedData } of compressedImages) {
    try {
      ...
      const newStream = PDFRawStream.of((obj as any).dict.clone(), compressedData);
